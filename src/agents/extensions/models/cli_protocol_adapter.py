...
def codex_provider_outputs_for_item(
    item: ThreadItem,
    *,
    cwd: str,
) -> list[TResponseOutputItem]:
    if isinstance(item, CommandExecutionItem):
        return _codex_command_execution_to_outputs(item, cwd=cwd)
    if isinstance(item, FileChangeItem):
        return _codex_file_change_to_outputs(item)
    if isinstance(item, McpToolCallItem):
        return [_codex_mcp_tool_call_to_output(item)]
    if isinstance(item, WebSearchItem):
        return [_codex_web_search_to_output(item)]
    return []
...
def stream_codex_item_event(
    *,
    ...
    if isinstance(item, CodexReasoningItem):
        events = session.sync_reasoning_text(
            item.id,
            item.text,
            label="Codex reasoning stream",
        )
        if isinstance(event, ItemCompletedEvent) and item.text:
            events.extend(session.finish_reasoning(item.id))
        return events

    if isinstance(item, AgentMessageItem):
        events = session.sync_message_text(
            item.id,
            item.text,
            label="Codex message stream",
        )
        if isinstance(event, ItemCompletedEvent) and item.text:
            events.extend(session.finish_message(item.id))
        return events

    if isinstance(event, ItemCompletedEvent):
        events: list[TResponseStreamEvent] = []
        for output_item in codex_provider_outputs_for_item(item, cwd=cwd):
            events.extend(session.append_instant_output_item(output_item))
        return events

    return []


def _codex_command_execution_to_outputs(
    item: CommandExecutionItem,
    *,
    cwd: str,
) -> list[TResponseOutputItem]:
    status = "completed" if item.status == "completed" else "incomplete"
    return [
        {
            ...
            "action": {
                "type": "exec",
                "command": ["sh", "-lc", item.command],
                "env": {},
                "working_directory": cwd,
            },
            "managed_by": "provider",
            "provider_data": {"vendor": "codex", "source_type": item.type},
        },
        {
            "type": "shell_call_output",
            "id": f"{item.id}_output",
            "call_id": item.id,
            "status": status,
            "output": [
                {
...
def _codex_file_change_to_outputs(item: FileChangeItem) -> list[TResponseOutputItem]:
    outputs: list[TResponseOutputItem] = []
    for index, change in enumerate(item.changes):
        call_id = f"{item.id}:{index}"
        operation = _codex_file_change_to_operation(change.path, change.kind)
        provider_data = {
            "vendor": "codex",
            "source_type": item.type,
            "change_kind": change.kind,
            "change_index": index,
            "diff_available": change.kind == "delete",
        }
        outputs.append(
            {
        ...
        outputs.append(
            {
                ...
                "output": _codex_file_change_output_message(change.path, change.kind, item.status),
                "managed_by": "provider",
                "provider_data": provider_data,
                "created_by": "provider",
            }
...
def _codex_file_change_to_operation(path: str, kind: str) -> dict[str, str]:
    if kind == "add":
        return {"type": "create_file", "path": path, "diff": ""}
    if kind == "delete":
        return {"type": "delete_file", "path": path}
    if kind == "update":
        return {"type": "update_file", "path": path, "diff": ""}
    raise ModelBehaviorError(f"Unsupported Codex file change kind: {kind}")


def _codex_file_change_output_message(path: str, kind: str, status: str) -> str:
    if status == "completed":
        if kind in {"add", "update"}:
            return (
                f"Codex reported a {kind} file change for {path}; "
                "diff was not exposed by the CLI."
            )
        return f"Codex deleted {path}."
    return f"Codex reported a failed {kind} file change for {path}."


def _codex_mcp_tool_call_to_output(item: McpToolCallItem) -> TResponseOutputItem:
    status = item.status if item.status != "in_progress" else "calling"
    result_output: str | None = None
    if item.result is not None and item.result.structured_content is not None:
        result_output = stringify_json_value(
            label=f"Codex MCP structured_content for {item.server}/{item.tool}",
    ...
    return {
        ...
        "id": item.id,
        "arguments": stringify_json_value(
            label=f"Codex MCP arguments for {item.server}/{item.tool}",
            value=item.arguments,
        ),
        "name": item.tool,
        "server_label": item.server,
        "status": status,
...
def _codex_web_search_to_output(item: WebSearchItem) -> TResponseOutputItem:
    return {
